...
def generate(data):
    ...
    data['params']['c_adr'] = "0x%0.8X" % generateAddress(data, {'name':'c','width':1,'type':'unsigned int'})
    data['params']['d_adr'] = "0x%0.8X" % generateAddress(data, {'name':'d','width':1,'type':'unsigned int'})
    data['params']['flag_adr'] = "0x%0.8X" % generateAddress(data, {'name':'flag','width':1,'type':'unsigned int'})

    data['params']['comparison_ab'] = random.choice(['<', '>', '<=', '>='])
    data['params']['comparison_cd'] = random.choice(['<', '>', '<=', '>='])
    data['params']['logic'] = random.choice(['&&', '||'])
