...
def generate(data):
    ...
    minmax = random.choice(['MAXIMUM', 'MINIMUM'])
    data['params']['minmax'] = minmax
    data['params']['varname'] = 'max' if minmax == 'MAXIMUM' else 'min'

    data['params']['x_adr'] = "0x%0.8X" % generateAddress(data, {'name':'X','width':10,'type':'unsigned int'})
    data['params']['max_adr'] = "0x%0.8X" % generateAddress(data, {'name':'max','width':1,'type':'unsigned int'})
